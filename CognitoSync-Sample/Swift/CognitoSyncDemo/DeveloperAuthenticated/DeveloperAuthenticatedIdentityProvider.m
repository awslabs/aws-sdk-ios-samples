...
- (AWSTask<NSString *> *)token {
    if ([self.client isAuthenticated]) {
        ...
        return [[self.client getToken:self.identityId logins:@{Constants.DEVELOPER_AUTH_PROVIDER_NAME: self.keychain[Constants.BYOI_PROVIDER]}] continueWithSuccessBlock:^id _Nullable(AWSTask * _Nonnull task) {
            if (task.result) {
