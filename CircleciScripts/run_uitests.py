...
from configure_aws_resources import configure_aws_resources, delete_aws_resources
...
for appname, app_config in vars(config_uitests.apps_to_uitest).items():
    ...
    if appname == "S3TransferUtility":
        continue
    try:
        cleanup_pod_repos = setup_pods(app_repo_root_directory = app_repo_root_directory,
                                       appname = appname,
    ...
    # configure_aws_resources(app_root_directory = '{0}/{1}'.format(app_repo_root_directory, app_config.path),
    #                         appname = appname,
    #                         cli_resources = app_config.cli_resources)

    app_root_directory = "{0}/{1}".format(app_repo_root_directory, app_config.path)
    try:
        build_and_uitest(circleci_root_directory = circleci_root_directory,
