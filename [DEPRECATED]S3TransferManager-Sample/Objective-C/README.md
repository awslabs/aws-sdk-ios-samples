...
1. Open `Info.plist` and update the following lines with the appropriate constants:
   ...
   **AWS --> CredentialsProvider --> CognitoIdentity --> Default --> PoolId** (use the IdentityPoolID you saved in the steps above)
   
   **AWS --> CredentialsProvider --> CognitoIdentity --> Default --> Region** (for example us-east-1)
    
    **AWS --> S3TransferManager --> Default --> Region** (for example us-east-1)
    
    **AWS --> S3 --> Default --> Region** (for example us-east-1)
